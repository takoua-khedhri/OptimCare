...
import 'passation.dart';
import 'package:reorderables/reorderables.dart';

class Evaluation extends StatefulWidget {
  final String patientId;

  const Evaluation({Key? key, required this.patientId}) : super(key: key);
...
class _EvaluationState extends State<Evaluation> {
  ...
  @override
  void dispose() {
    _commentairesControllers.values.forEach(
      ...
      showDialog(
        ...
        builder:
            (context) => AlertDialog(
              ...
              content: const Text(
                'Tous les besoins semblent satisfaits. Souhaitez-vous tout de même créer un plan de soins?',
              ),
              actions: [
                TextButton(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle:
            true, // Ceci est nécessaire pour centrer le titre dans l'AppBar
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: const Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.medical_services_outlined, size: 22),
                          SizedBox(width: 12),
                          Text(
                            'Générer un plan de soin',
                            style: TextStyle(
                              fontSize: 17,
...
class _PlanDeSoinsPageState extends State<PlanDeSoinsPage> {
  ...
  final Map<String, List<String>> _interventionsParDiagnostic = {
    ...
    'Risque de chute': ['Prévention chutes', 'Environnement sécurisé'],
    ...
    'Diminution de l'
        'implication dans des activités de loisirs': [
      'Activités ludiques',
      'Sorties',
    ],
  ...
  List<String> _getDiagnosticsForBesoin(String nomBesoin) {
    switch (nomBesoin) {
      ...
      case 'Se vêtir et se dévêtir':
        return ['Déficit des soins personnels'];
      ...
      case 'Communiquer avec ses semblables':
        return ['Motivation à ameliorer sa communication'];
      case 'Agir selon ses croyances et ses valeurs':
        return ['Motivation à ameliorer son bien-etre spirtuel'];
      case 'S\'occuper en vue de se réaliser':
        return ['Exercice inefficace du role'];
  ...
  List<String> _getObjectifForDiagnostic(String diagnostic) {
    switch (diagnostic) {
      case 'Mode de respiration inefficace':
        return [
          'maintiendra une fréquence respiratoire entre 12 et 20/min dans les 24h.',
        ];
      ...
      case 'Insomnie':
        return [
          's\'endormira dans un délai de 30 minutes chaque nuit pendant 3 jours',
        ];
      case 'Mobilité physique réduite':
        return ['marchera 20 mètres avec aide dans les 72h'];
      case 'Mobilité réduite au fauteuil roulant':
        return [
          'pourra se déplacer seule en fauteuil sur 10 mètres d\'ici 48h',
        ];
  ...
  void _moveItem(Besoin draggedBesoin, int newIndex) {
    setState(() {
      final oldIndex = _orderedBesoins.indexWhere(
        (b) => b.id == draggedBesoin.id,
      );
      if (oldIndex != -1) {
  ...
  Future<void> _enregistrerPlanDeSoins() async {
    ...
    try {
      ...
      // Redirection vers PassationSoins après l'enregistrement
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => PassationSoins(patientId: widget.patientId),
        ),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    for (var besoin in _orderedBesoins) {
      if ((_diagnosticsSelectionnes[besoin.id]?.isEmpty ?? true)) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Veuillez sélectionner au moins un diagnostic pour ${besoin.nom}',
            ),
      ...
      if (_evaluations[besoin.id] == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Veuillez sélectionner une évaluation pour ${besoin.nom}',
            ),
            backgroundColor: Colors.orange,
          ),
        );
        ...
        if (allObjectives.isNotEmpty) ...[
          ...allObjectives.map((objectif) {
            return CheckboxListTile(
              title: Text(objectif),
              value:
                  !_usingCustomObjective[besoin.id]! &&
  ...
  Widget _buildInterventionsSection(Besoin besoin) {
    ...
    final interventions =
        selectedDiagnostics
            .expand(
              (diagnostic) => _interventionsParDiagnostic[diagnostic] ?? [],
            )
        ...
        if (interventions.isNotEmpty) ...[
          ...interventions.map((intervention) {
            return CheckboxListTile(
              ...
              value:
                  _interventionsSelectionnees[besoin.id]?.contains(
                    intervention,
                  ) ??
                  false,
              onChanged: (selected) {
                setState(() {
                  if (selected == true) {
                    ...
                    _interventionsSelectionnees[besoin.id]?.remove(
                      intervention,
                    );
                  }
                });
  ...
  Widget _buildEvaluationSection(Besoin besoin) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...['En cours', 'Amélioration', 'Stagnation', 'Aggravation'].map((
          value,
        ) {
    ...
    return Column(
      children: [
        ...
        _buildMobileSection('Interventions', [
          _buildInterventionsSection(besoin),
        ]),
        const SizedBox(height: 12),
